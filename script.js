...
document.querySelector("#page4>h1").textContent.split(" ").forEach(function(dets){
  clutterr += `<span> ${dets} </span>`
  document.querySelector("#page4>h1").innerHTML=clutterr;
})
...
gsap.timeline({
  ...
  .to(num, {var:60 , duration: 1200000, ease:"slowly", onUpdate:changeNumber})
  .to({}, {duration:1200000})
  

function changeNumber() {
  val=document.getElementById("per");
  val.textContent=`${(num.var).toFixed()}%`;
...
function canvas4(){
const canvas = document.querySelector(".right11>canvas");
const context = canvas.getContext("2d");
...
// document.querySelector(".left11-data").textContent.split("").forEach(function(dets){
//   clutter += `<span>${dets}</span>`
//   document.querySelector(".left11-data").innerHTML=clutter;
// })
